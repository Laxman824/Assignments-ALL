...
def TSP_with_annealing(matrix):                              
  ...
  #  print("The Tsp path computed with simulated annealing ")
   ...
   print(soln)
   plt.title(f"(SIMULATED ANNEALING)Relation blw iterations and solution ")
...
def main(n,M):
  ...
  TSP_with_annealing(rangen(n,M))
def rangen(v,M):
  res=[]
  for i in range(v):
    res.append([])
    for j in range(v):
      res[i].append(np.random.randint(0,M))
  return res

if __name__ == "__main__":
  n=int(input("ENTER THE NUMBER OF VERTICES : "))
  M=int(input("ENTER THE VERTEX LIMIT VALUE RANDOMLY GENERATED IN BETWEEN(0-1000) : "))

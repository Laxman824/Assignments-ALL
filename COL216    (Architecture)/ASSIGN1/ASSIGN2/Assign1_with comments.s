...
bl fgets	     @branch and link fgets takes input from terminal
...
cmploop:
	...
	beq label      @branch equal then goto Label  at this point if string is lax and pus strnotnull
	cmp r1,r2      @cmpare both strings 
	bgt label	@branchifgreatrthan  goto Lbael   here comprision and if greater string is (lax or push) then false here 
	blt L		
	add r3,r3,#4   @ str1++ here pointers has to be updated so,r3 and r4 increment chesam
	add r4,r4,#4	@str2++
